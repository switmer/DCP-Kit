...
| `colorCandidateDepth` | 1.00 | 0.20 | 156 usable color candidates |
| `fontSizeCoverage` | 1.00 | 0.15 | 162 font-size tokens |
| `spacingCoverage` | 1.00 | 0.10 | 98 spacing tokens (scale+section) |
| `responsiveSignal` | 0.00 | 0.10 | no clamp() or @media rules observed |

## Binding confidence distribution

- **13** roles bound. High-saliency (≥0.85): **0**. Medium (0.65–0.85): **9**. Low (<0.65): **4**.
...
- `bg.default` = `#0000000a` — validity **0.25**. Failed rules:
  - *surface-role-needs-opaque*: role bg.*/border.* expects opaque fill; observed alpha 0.04
- `bg.elevated` = `#ffffff1a` — validity **0.25**. Failed rules:
  - *surface-role-needs-opaque*: role bg.*/border.* expects opaque fill; observed alpha 0.10
