...
import { DCPDiagnosticsProvider } from './diagnosticsProvider';
import { DCPCompletionProvider } from './completionProvider';
import { DCPHoverProvider } from './hoverProvider';

let registryClient: RegistryClient;
let diagnosticsProvider: DCPDiagnosticsProvider;
...
export function activate(context: vscode.ExtensionContext) {
    ...
    registryClient = new RegistryClient(outputChannel);
    
    // Initialize providers
    diagnosticsProvider = new DCPDiagnosticsProvider(registryClient, outputChannel);
...
function setupProviders(context: vscode.ExtensionContext) {
    ...
    const completionProvider = new DCPCompletionProvider(registryClient);
    ...
    const hoverProvider = new DCPHoverProvider(registryClient);
    context.subscriptions.push(
...
function debounceValidation(document: vscode.TextDocument) {
...
function setupCommands(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('dcp.validateFile', async () => {
            const activeEditor = vscode.window.activeTextEditor;
            if (activeEditor && shouldValidateFile(activeEditor.document)) {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('dcp.refreshRegistry', async () => {
            await loadRegistry();
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('dcp.openRegistry', async () => {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('dcp.extractRegistry', async () => {
...
async function showRegistrySetupPrompt() {
    const action = await vscode.window.showInformationMessage(
        'No DCP registry found. Extract components from your codebase to enable design system validation.',
        'Extract Now',
        'Configure Path',
        'Learn More'
    );
