...
exports.DCPHoverProvider = void 0;
const vscode = require("vscode");
class DCPHoverProvider {
    ...
    provideHover(document, position, token) {
        ...
        switch (hoverContext.type) {
            ...
            case 'prop':
                return this.createPropHover(hoverContext.componentName, hoverContext.propName, wordRange);
            default:
                return null;
        }
    }
    analyzeHoverContext(word, linePrefix, lineText, position, document) {
    ...
    extractTokenFromClassName(className, hoveredWord) {
    ...
    findComponentNameInLine(lineText) {
        const match = lineText.match(/<(\w+)/);
        return match ? match[1] : null;
    }
    createComponentHover(componentName, range) {
        const component = this.registryClient.getComponent(componentName);
        if (!component) {
    ...
    createTokenHover(tokenName, range) {
        const token = this.registryClient.findToken(tokenName);
        if (!token) {
    ...
    createPropHover(componentName, propName, range) {
        const component = this.registryClient.getComponent(componentName);
