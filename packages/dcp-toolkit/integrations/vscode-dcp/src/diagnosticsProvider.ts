...
export class DCPDiagnosticsProvider {
    private diagnosticsCollection: vscode.DiagnosticCollection;
    private registryClient: RegistryClient;
    private outputChannel: vscode.OutputChannel;

    constructor(registryClient: RegistryClient, outputChannel: vscode.OutputChannel) {
    ...
    async validateDocument(document: vscode.TextDocument): Promise<void> {
        ...
        try {
            ...
            this.outputChannel.appendLine(
                `Validation complete: ${diagnostics.length} issues found`
            );
    ...
    private convertToDiagnostics(
        ...
        const validationLevel = config.get('validationLevel', 'error') as string;
    ...
    private createDiagnostic(
        ...
        try {
            ...
            const diagnostic = new vscode.Diagnostic(
                range,
                issue.message || 'Design system validation issue',
                severity
            );
    ...
    private findEndColumn(lineText: string, startColumn: number, issue: any): number {
