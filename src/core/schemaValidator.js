...
export class SchemaValidator {
  constructor(options = {}) {
    ...
    this.schemasDir = options.schemasDir || './schemas';
  ...
  loadSchemas() {
    if (!fs.existsSync(this.schemasDir)) {
      if (this.verbose) {
        console.warn(`⚠️ Schema directory not found: ${this.schemasDir}`);
      }
      return;
    }
    ...
    for (const schemaFile of schemaFiles) {
      try {
        ...
        const schemaName = schemaFile.replace('.schema.json', '').replace('.json', '');
