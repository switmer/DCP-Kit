...
### 🐛 Bug Fixes
- **CRITICAL**: Fixed duplicate baseUrl in Browse UI component URLs
  - Component URLs now relative (`/r/ui/button`) instead of absolute
  - Fixes "Failed to load component details" modal crash
  - Before: `http://localhost:7401http://localhost:7401/r/ui/button` (404)
  - After: `/r/ui/button` (resolves correctly)

## [3.0.2] - 2025-11-03

### 🐛 Bug Fixes
- **CRITICAL**: Fixed Browse UI crash when opening components without dependencies
  - Added null-safety checks for `component.dependencies` and `component.peerDependencies`
  - Fixes "deps is not iterable" TypeError

## [3.0.1] - 2025-11-03

### 🐛 Bug Fixes
- **CRITICAL**: Fixed Browse UI static assets not loading (CSS/JS 404 errors)
  - Added explicit routes for `/browse.css` and `/browse.js` in serve-registry
  - Files served from packs directory with proper MIME types and caching
