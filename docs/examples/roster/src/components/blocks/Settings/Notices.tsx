...
import { NoteType } from '../CallSheet/Notes/Manage';
...
export const Notices = ({ company }: { company?: CompanyType | null }) => {
  ...
  const addNewNote = (type: NoteType) => {
    ...
    setContainers((prevContainers) => {
      ...
      if (prevContainers[type] === undefined) {
        console.warn(`Container for note type "${type}" does not exist`);
  ...
  const deleteNote = async () => {
    if (current !== null) {
      ...
      setContainers((prevContainers) => {
        const noteType = noteToDelete.type as NoteType;
        // check if the container exists.
        if (prevContainers[noteType] === undefined) {
          console.warn(`Container for note type "${noteType}" does not exist`);
          return prevContainers;
        }

        return {
          ...prevContainers,
          [noteType]: updateNotes(prevContainers[noteType]),
        };
      });
  ...
  const updateNote = useCallback(
    (field: 'title' | 'note' | 'acknowledgeable' | 'type' | 'isHighlighted', value: string | boolean) => {
      if (!!currentNote) {
        setContainers((prevContainers) => {
          // check if the container for the current note type exists.
          const noteType = currentNote.type as NoteType;
          const containerExists = prevContainers[noteType] !== undefined;
          ...
          const updatedContainers = {
            ...prevContainers,
            [noteType]: prevContainers[noteType].map((note) =>
