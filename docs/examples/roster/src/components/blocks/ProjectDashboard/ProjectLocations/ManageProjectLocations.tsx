...
export const ManageProjectLocations: FC<Props> = (props) => {
  ...
  const [selectedLocationMapUrl, setSelectedLocationMapUrl] = useState<
    string | null
  >(null);
  ...
  const [newLocationInstructions, setNewLocationInstructions] = useState<
    string | null
  >(null);
  ...
  useEffect(() => {
    // If passed selectedLocation directly as a prop, use it
  ...
  useEffect(() => {
    ...
    //-- "edit" view, when props.open is a location object.
    if (typeof props.open === "object" && props.open !== null) {
      setSelectedLocation(props.open);

      //-- update form fields with location data.
      if (props.open) {
        const loc = props.open;
        setNewLocationName(loc.name || "");
        setNewLocationAddress(loc.address || "");
        setNewLocationType((loc.type as string) || "shoot");
        setNewLocationDescription(loc.description || null);
        setNewLocationInstructions(loc.instructions || null);
      }
    ...
    //-- list view (or add if no locations).
    if (
  ...
  const handleSave = async () => {
    ...
    switch (view) {
      case "add":
        ...
        const placesJson = await getGooglePlaceDetailsClientSide(
          newLocationAddress
        );
        ...
        const { error: projectLocationResError } = await supabase
          ...
          if (existingLocation[0]) {
            ...
            const { error: updateLocationError } = await supabase
              ...
              .update({
                ...
                ...(newLocationType !== selectedLocation.type
                  ? { type: newLocationType }
                  : {}),
                ...(newLocationInstructions === null
                  ? {}
                  : { instructions: newLocationInstructions }),
            ...
            const { data: newLocation, error: newLocationError } =
              await supabase
                .from("location")
                .insert({
                  company: user.id,
            ...
            if (!newLocation || newLocationError) {
              toast.error(
                "Something went wrong creating new location record. Please try again."
              );
            ...
            const { data: newProjectLocation, error: newProjectLocationError } =
              await supabase
                .from("project_location")
                .insert({
            ...
            if (newProjectLocationError) {
              toast.error(
                "Something went wrong creating new call sheet location record."
              );
  ...
  const deleteLocation = async (location: ProjectLocationWithAddress) => {
    if (!location) return;
