...
export const ManageFiles: React.FC<ManageFilesProps> = ({
  open,
  onClose,
  files,
  callSheet,
  ...
  const addNew = (type: FileType, selectedFile: File) => {
    const newFile = createNewFile(type, callSheet, project, activeCompany);
          ...
          .map(async (f, index) => {
            ...
            if (f.id < 0) {
              ...
              const sanitizedTitle = sanitizeFilename(
                f.title || f.file.name || "Untitled"
              );

              const { data, error } = await supabase.storage
                .from("attachments")
                .upload(
                  [activeCompany, f.project, f.call_sheet, sanitizedTitle]
                    .filter(Boolean)
                    .join("/"),
                  f.file,
                  {
