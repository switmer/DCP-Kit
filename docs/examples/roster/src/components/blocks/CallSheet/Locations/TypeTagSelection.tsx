...
export const TypeTagSelection: React.FC<Props> = (props) => {
  const [modalOpen, setModalOpen] = useState(false);
  const [customTag, setCustomTag] = useState<string>("");
  const [_refreshKey, setRefreshKey] = useState(0);
  const [selected, setSelected] = useState<string>(
    splitAndLowercaseFirst(props.value) ?? ""
  );
  ...
  const handleFormSubmit = (values: FormikValues) => {
    ...
    if (props.addCustomTagCallback && values.custom_tag !== "") {
      ...
      props.setNewLocationType(newValue);
    ...
    <>
      ...
      <div className="relative">
        <div
          className={cn(
            props.className,
            "flex items-center gap-[6px] flex-wrap"
          )}
          onClick={() => {
                ...
                return (
                  <div
                    onClick={() => {
                      ...
                      props.setNewLocationType(splitAndLowercaseFirst(s));
                    ...
                    className={cn(
                      ...
                      splitAndLowercaseFirst(s) ===
                        splitAndLowercaseFirst(selected) &&
                  ...
                  >
                    {splitAndLowercaseFirst(s) ===
                      splitAndLowercaseFirst(selected) &&
                      getLocationTypeIcon(splitAndLowercaseFirst(s))}
                    {s}
                  </div>
                );
              })}
